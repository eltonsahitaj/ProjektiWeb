...
class KategoriaCRUD extends db
{
  ...
  public function getEmriKategoris()
    ...
    public function insertoKategorinLajmit()
    {
        try {
            ...
            $sql = "INSERT INTO `kategoriaanimes`(`emriKategoris`, `pershkrimiKategoris`) VALUES (?,?)";
    ...
    {
        try {
            $sql = "UPDATE `kategoriaanimes` set `emriKategoris` = ?, `pershkrimiKategoris` = ? WHERE kategoriaID = ?";
            $stm = $this->dbConn->prepare($sql);
    ...
    {
        try {
            $sql = "SELECT * FROM kategoriaanimes";
            $stm = $this->dbConn->prepare($sql);
            $stm->execute();
    ...
    public function shfaqKategorinSelektim()
    {
        try {
            ...
            echo '<select class="form-input" name="kategoria">
                <option value="Anime">Zgjedhni Kategorin</option>
            ';
    ...
    {
        try {
            $sql = "DELETE FROM kategoriaAnimes WHERE kategoriaID = ?";
            $stm = $this->dbConn->prepare($sql);
